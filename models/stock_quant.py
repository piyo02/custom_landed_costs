    ...
    def count_cost(self):
        ...
        if len(self.history_ids) > 1:
            for hist_id in self.history_ids:
                ...
                landed_costs = self.env['custom.landed.cost'].search([('picking_ids.name', '=', picking_name)])
            ...
            landed_cost = self.env['custom.landed.cost'].search([('picking_ids.name', '=', picking_name)])
